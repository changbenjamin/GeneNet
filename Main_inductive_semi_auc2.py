...
def evaluateResults(test_neg_agent0,test_neg_agent1,test_prob_agent0,test_prob_agent1,test_pos):
    ...
    y_true = [1] * len(test_pos[0]) + [0] * len(test_neg[0])
    ensemble_y_score = [test_prob_agent0[i] + test_prob_agent1[i] for i in range(len(test_prob_agent0))]
    ensemble_auc = compute_auc(y_true, ensemble_y_score)
    agent0_auc = compute_auc(y_true, test_prob_agent0)
    agent1_auc = compute_auc(y_true, test_prob_agent1)
    
    print("Ensemble AUC: " + str(ensemble_auc))
    print("Agent0 AUC: " + str(agent0_auc))
    print("Agent1 AUC: " + str(agent1_auc))
