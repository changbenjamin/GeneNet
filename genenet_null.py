...
if args.data_name is not None:
    ...
    # Shuffle labels for the null trial
    total_pos_shuffled = shuffle(total_pos, random_state=42)
    total_neg_shuffled = shuffle(total_neg, random_state=42)

    # Split dataset into training and testing
    train_pos_0, test_pos_0 = train_test_split(total_pos_shuffled[0], train_size=args.training_ratio)
    train_pos_1, test_pos_1 = train_test_split(total_pos_shuffled[1], train_size=args.training_ratio)
    train_neg_0, test_neg_0 = train_test_split(total_neg_shuffled[0], train_size=args.training_ratio)
    train_neg_1, test_neg_1 = train_test_split(total_neg_shuffled[1], train_size=args.training_ratio)
...
def evaluateResults(test_neg_agent0,test_neg_agent1,test_prob_agent0,test_prob_agent1,test_pos):
    ...
    ensemble_auc = roc_auc_score(trueList, ensembleProb)
    print(f"Ensemble AUC: {ensemble_auc}")
    
    # Save the ROC curve plot for the ensemble results
    filename = f'{bio_mean:.2f}_{bio_std_dev:.2f}_roc_curve.png'
    save_roc_curve(trueList, ensembleProb, filename)
